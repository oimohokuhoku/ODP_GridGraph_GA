...
Individual EmbeddPartialGraph::operator() (const Individual& indiv0, const Individual& indiv1, const EmbeddMap& embeddMap) {
    Individual newIndiv;
    newIndiv.clear();

    inheritEdgesWithEmbeddMap(newIndiv, indiv0, indiv1, embeddMap);
    inheritShareEdges(newIndiv, indiv0, indiv1);
    inheritRandomEdges(newIndiv, indiv0, indiv1);
...
void EmbeddPartialGraph::inheritRandomEdges(Individual& newIndiv, const Individual& indiv0, const Individual& indiv1) {
    std::vector<int> nodeSequence(newIndiv.numNode());
    for(int i = 0; i < nodeSequence.size(); ++i) nodeSequence[i] = i;
    std::shuffle(nodeSequence.begin(), nodeSequence.end(), Random::engine());
    ...
    for(int i = 0; i < nodeSequence.size(); ++i) {
        ...
        const Individual* srcIndiv;
        if(Random::roulette(0.5)) srcIndiv = &indiv0;
        else                      srcIndiv = &indiv1;
