...
void GeneticAlgorithm::progressGeneration(
    unique_ptr<CopySelect>& copySelect,
    unique_ptr<Crossover>& crossover,
    unique_ptr<Mutate>& mutate,
    unique_ptr<SurvivorSelect>& survivorSelect,
    mt19937& random
) {
    ...
    while(numNextIndivs < nextGroup.population()) {
		...
		Group survivor = survivorSelect->select(childs, parents);
